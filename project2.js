...
class MeshDrawer {
	...
	constructor() {
		...
		this.lightLoc = gl.getUniformLocation(this.prog, 'lightPos');
	...
	draw(trans) {
		...
		updateLightPos();
		gl.uniform3fv(this.lightLoc, [lightX,lightY,-5])
...
const meshFS = `
			...
			void main()
			{
				...
				if(showTex && enableLighting){
					...
					float diffusion = max(dot(normalized, lightDirection), 0.0);
					vec3 diffuse = diffusion * texColor.rgb;

					vec3 ambientColor = ambient * texColor.rgb;

					vec3 finalView = (ambientColor + diffuse)*texColor.rgb;
					gl_FragColor = vec4(finalView, 1);
